...
\usepackage{booktabs, xcolor, colortbl, longtable}
...
\newcommand{\type}[1]{\texttt{#1}} %use for variables in S
...
\caption{\label{NumerComTa}Useful Commands When Working with Numeric
Vectors}\\  %[-2ex]
% \hline
\toprule
...
\type{fivenum(x)} & Computes the smallest value, the lower hinge, the median,
the upper hinge, and the largest value of a vector $x$ in \prog{R}\\
\type{floor(x)} & Returns a numeric vector containing the largest integers not
...
\type{quantile(x)} & Computes the quantiles of a data set stored in a vector $x$\\
\type{range(x)} & Returns the smallest and largest values in $x$\\
\type{rbind(x,y)} & Joins vectors $x$ and $y$ as
rows of vectors\\
...
\type{sd(x)} & Computes the sample standard deviation of $x$ in \prog{R}\\
\type{seq(x,y,n)} & Creates a sequence of numbers  from $x$ to $y$ with incremental value $n$\\
\type{sin(x)} &Returns the sine for all values in $x$\\
\type{sqrt(x)} &Computes the square root for all values in $x$\\
\type{stdev(x)} &  Computes the sample standard deviation of $x$ in \prog{S-PLUS}\\
\type{sum(x)} &The sum of all the values in  $x$\\
\type{summary(x)} & Returns the minimum, the first quartile, the median, the
mean, the third quartile, and the maximum of $x$\\
\type{tan(x)}& Returns
the tangent for all values in $x$\\
\type{var(x)} &Computes the sample variance of $x$\\
\type{which(x==n)} & Give the index of number $n$ in
vector $x$
\end{longtable}
